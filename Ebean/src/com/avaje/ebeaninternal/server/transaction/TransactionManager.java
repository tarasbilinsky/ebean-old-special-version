	...
	public void notifyOfCommit(SpiTransaction transaction) {
      ...
      if (commitDebugLevel >= 1) {
        logger.info("Transaction ["+transaction.getId()+"] commit");
			...
			ByteArrayOutputStream ss = new ByteArrayOutputStream(1024);
			ex.printStackTrace(new PrintStream(ss));
			if(!ss.toString().contains("com.avaje.ebeaninternal.server.deploy.BeanPropertyAssocOne.cacheDelete"))logger.error( m, ex);
		}
	}

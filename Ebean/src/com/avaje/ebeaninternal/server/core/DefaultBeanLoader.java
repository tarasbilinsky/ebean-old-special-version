  ...
  public void loadMany(LoadManyRequest loadRequest) {
    ...
    for (int i = 0; i < batch.size(); i++) {
      ...
      if (bc.checkEmptyLazyLoad()) {
          logger.trace("BeanCollection after load was empty. Owner:" + batch.get(i).getOwnerBean());
  ...
  public void loadMany(BeanCollection<?> bc, LoadManyContext ctx, boolean onlyIds) {
    ...
    if (beanCollection != null) {
      if (beanCollection.checkEmptyLazyLoad()) {
          logger.trace("BeanCollection after load was empty. Owner:" + beanCollection.getOwnerBean());
